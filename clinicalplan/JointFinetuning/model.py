...
For single-outcome use, instantiate with `num_tasks=1`.
"""
...
class CustomBioClinicalBertForCombinedLearning(BertForMaskedLM):
    ...
    For single-outcome (joint) fine-tuning, all examples carry `task_id=0`
    and only `auxiliary[0]` is exercised — the loop becomes a single-task BCE.
    """
