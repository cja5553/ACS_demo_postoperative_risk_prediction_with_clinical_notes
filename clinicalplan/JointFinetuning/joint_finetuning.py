...
Exposes three functions:
    ...
    get_pseudo_data()
        Generate 1000 synthetic preoperative notes with a single binary
        `outcome` column driven by realistic clinical features. Mirrors the
        notes produced by `MultiTaskFinetuning.get_pseudo_data` but exposes
        only one outcome (high cardiac risk) for single-outcome workflows.
"""
...
def get_pseudo_data():
    """
    Generate 1000 synthetic preoperative notes with a single binary `outcome`
    column for testing the joint single-outcome workflow.

    The notes are identical in style to those from
    `MultiTaskFinetuning.get_pseudo_data`. The single `outcome` column reflects
    a feature-driven "high cardiac risk" label, derived from:
      - cardiac procedures (CABG, AVR, carotid endarterectomy)
      - age >= 70
      - ASA class >= 3
      - current smoker

    Use this to smoke-test `joint_finetune` before plugging in real clinical data.

    Returns
    -------
    pandas.DataFrame
        Columns: "text" (str), "outcome" (int 0 or 1).
    """
    rng = np.random.default_rng(0)

    # ---- building blocks -----------------------------------------------------
    # Each procedure carries risk flags so we can build the outcome from it.
    procedures = [
    ...
    def synthesize_row(rng):
        """Build a single synthetic note + its high-cardiac-risk outcome."""
        ...
        is_smoker = bool(rng.binomial(1, 0.20))
        is_obese = bool(rng.binomial(1, 0.30))
        ...
        # ---- single outcome: high cardiac risk ------------------------------
        # Driven by cardiac procedure + age >= 70 + ASA >= 3 + smoker.
        logit = -2.5
        if flags["cardiac"]:
            logit += 2.5
        if age >= 70:
        ...
        outcome = int(rng.binomial(1, _sigmoid(logit)))

        # ---- compose the note (same style as MTL pseudo data) ---------------
        sentences = [
    ...
    return pd.DataFrame({"text": texts, "outcome": outcomes})
